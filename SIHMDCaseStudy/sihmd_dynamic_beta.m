...
function validateParameters(params)
    ...
    if any([params.beta1, params.beta2, params.gamma, params.alpha] <= 0)
...
function result = sihmd_agent_model(N, params)
    ...
    while ~isempty(I) && t < params.tmax
        % Check if we need to change beta
        if t >= params.t1
            beta = params.beta2;
        else
            beta = params.beta1;
        ...
        infection_rate = params.pI * beta * nS * nI / N;  % S to I rate
...
function det_result = solve_deterministic_sihmd(params)
    ...
    % Define time-dependent beta function
    beta_func = @(t) (t < params.t1) * params.beta1 + (t >= params.t1) * params.beta2;

    % Initial conditions vector
    y0 = [params.s0; params.i0; params.h0; params.m0; params.d0];

    % Define the ODE system using time-dependent beta
    ode_system = @(t, y) [
        -beta_func(t) * y(1) * y(2) * params.pI + params.gamma * y(2) * params.pIS + params.alpha * y(3) * params.pHS; % ds/dt
        beta_func(t) * y(1) * y(2) * params.pI - params.gamma * y(2);                                                  % di/dt
    ...
    % Calculate R0 using initial beta
    det_result.R0 = params.pI * params.beta1 / params.gamma;
...
function plot_comparison(results, N_values, det_result, params)
    ...
    param_text = sprintf('R₀=%.2f, β_1=%.2f, β_2=%.2f, γ=%.2f, α=%.2f\np_I=%.2f, p_IS=%.2f, p_IH=%.2f, p_IM=%.2f, p_ID=%.2f\np_HS=%.2f, p_HM=%.2f, p_HD=%.2f', ...
        det_result.R0, params.beta1, params.beta2, params.gamma, params.alpha, ...
