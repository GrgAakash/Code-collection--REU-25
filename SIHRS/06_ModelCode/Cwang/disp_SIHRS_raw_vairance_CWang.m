...
function sihrs_renormalized_simulation()
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     params.N_values = [ 1600, 10000, 50000]; % Population sizes - matching SIHRS.m
    
  % params.N_values = [300, 1600, 10000];

 %params.N_values = [1600, 1600, 3000];
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%% CWang: I am going to change to R_N 1. So we check if the
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% denominator which is G is non-zero. 
...
%result.D1_squared(valid_S) =  (1/N) * (beta * params.pSI * result.I_mean(valid_S) .* result.S_mean(valid_S) + ...
                            %        params.lambda * params.pRS * result.R_mean(valid_S)  )./((-beta * params.pSI * result.I_mean(valid_S) .* result.S_mean(valid_S) + ...
                            %        params.lambda * params.pRS * result.R_mean(valid_S)  ).^2);
   ...
   % result.D2_squared(valid_I) = (1/N) * ((params.gamma * (params.pIH + params.pIR + params.pID)) ./ result.I_mean(valid_I) + ...
                               %      beta * params.pSI * result.S_mean(valid_I)  );
   


    % result.D2_squared(valid_I) = (1/N) * ((params.gamma * (params.pIH + params.pIR + params.pID)) .* result.I_mean(valid_I) + ...
                             %        beta * params.pSI * result.S_mean(valid_I) .* result.I_mean(valid_I))./((-(params.gamma * (params.pIH + params.pIR + params.pID)) .* result.I_mean(valid_I) + ...
                              %       beta * params.pSI * result.S_mean(valid_I) .* result.I_mean(valid_I)).^2);
...
%result.D3_squared(valid_H) = (1/N) * ((params.alpha * (params.pHR + params.pHD)) .* result.H_mean(valid_H) + ...
                              %      params.gamma * params.pIH .* result.I_mean(valid_H)  )./(((-params.alpha * (params.pHR + params.pHD)) .* result.H_mean(valid_H) + ...
                             %       params.gamma * params.pIH .* result.I_mean(valid_H)  ).^2);
...
result.D3_squared(valid_H) = (1/N) * ((params.alpha * (params.pHR + params.pHD)) .* result.H_mean(valid_H) + ...
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
    ...
    % result.D4_squared(valid_R) = (1/N) * (params.lambda * params.pRS ./ result.R_mean(valid_R) + ...
     ...
     % result.D4_squared(valid_R) = (1/N) * (params.lambda * params.pRS .* result.R_mean(valid_R) + ...
                         ...
                         %           params.alpha * params.pHR * result.H_mean(valid_R))./(((-params.lambda * params.pRS .* result.R_mean(valid_R) + ...
                                %    params.gamma * params.pIR * result.I_mean(valid_R)   + ...
                                %    params.alpha * params.pHR * result.H_mean(valid_R))).^2) ;
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    V5 =     (params.pID * params.gamma * result.I_mean + params.pHD * params.alpha * result.H_mean);
 ...
 % result.D5(valid_D) = sqrt(V5(valid_D)) ./(params.pID * params.gamma * result.I_mean(valid_D)+ params.pHD * params.alpha * result.H_mean(valid_D)  );
 
 % result.D5(valid_D) =(1/sqrt(N)) * 1./sqrt(V5(valid_D))  ;
 result.D5(valid_D) =(1/sqrt(N)) .*  sqrt(V5(valid_D)   );
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    result.N = N;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    hold on;
    for idx = 1:length(params.N_values)
        plot(t, results{idx}.D1, 'Color', params.colors{idx}, 'LineWidth', 3);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    hold on;
    for idx = 1:length(params.N_values)
        plot(t, results{idx}.D2, 'Color', params.colors{idx}, 'LineWidth', 3);
    end
...
set(gca,'FontSize',30);
    ...
    hold on;
    for idx = 1:length(params.N_values)
        plot(t, results{idx}.D3, 'Color', params.colors{idx}, 'LineWidth', 4);
    end
...
set(gca,'FontSize',30);
    ...
    % axis square;
       ...
       legend_labels = arrayfun(@(n) sprintf('N = %d', n), params.N_values, 'UniformOutput', false);
    legend(legend_labels, 'Location', 'northeast', 'FontSize', 20);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %title(sprintf('Renormalized Std Dev - Recovered (R_0 = %.2f)', R0));
      ...
      xlabel('Time (days)','FontSize', 20);
    ylabel('$\mathcal{D}_N^{(4)}(t)$', 'Interpreter','latex', 'FontSize', 20);
    grid on;
...
xlim([10, 1000]);
 ...
 set(gca,'FontSize',20);
   ...
   % title(sprintf('Renormalized Std Dev - Dead (R_0 = %.2f)', R0));
    ...
    xlabel('Time (days)','FontSize', 30);
    ylabel('$\mathcal{D}_N^{(5)}(t)$', 'Interpreter','latex', 'FontSize', 20);
    grid on;
