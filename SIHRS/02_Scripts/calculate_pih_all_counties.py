...
class PIHCalculator:
    """Calculate P(IH) for all U.S. counties using Carson City methodology."""
    ...
    def load_nyt_data(self, filename='../01_RawData/nyt_covid_data.csv'):
        ...
        # Clean FIPS codes - handle NaN and convert to 5-digit string
    ...
    def calculate_active_cases(self, fips):
        """
        Calculate active cases for a county.
        
        CRITICAL: Use cases[t] - cases[t-14], NOT rolling sum!
        """
        ...
        # CRITICAL: Exact Carson City formula
    ...
    def calculate_pih_for_county(self, hosp_file):
        """Calculate P(IH) for a single county."""
        try:
            ...
            for i in range(2, len(hosp_df)):
                ...
                if 10 <= date_diff <= 15:
                    ...
                    if len(matching) > 0:
                        ...
                        # Include zeros (when hospitalized = 0) to match Carson City methodology
    ...
    def save_results(self):
        ...
        df.to_csv(output_dir / 'pih_all_counties.csv', index=False)
        logger.info(f"Saved: {output_dir / 'pih_all_counties.csv'}")

        # Save state-level summary
        state_summary = df.groupby('state').agg({
            'mean_pih': ['mean', 'median', 'std', 'count'],
            'n_calculations': 'sum'
        }).round(6)
        state_summary.columns = ['_'.join(col).strip() for col in state_summary.columns]
        state_summary = state_summary.reset_index().sort_values('mean_pih_mean', ascending=False)
        state_summary.to_csv(output_dir / 'pih_by_state.csv', index=False)
        logger.info(f"Saved: {output_dir / 'pih_by_state.csv'}")
        ...
        summary.to_csv(output_dir / 'pih_summary_statistics.csv', index=False)
        logger.info(f"Saved: {output_dir / 'pih_summary_statistics.csv'}")
...
def main():
    logger.info("="*70)
    logger.info("P(IH) CALCULATION - CARSON CITY METHODOLOGY")
    logger.info("="*70)
