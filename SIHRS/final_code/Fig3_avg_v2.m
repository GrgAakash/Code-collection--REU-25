%% Figure 3: Path Averages

clear all; close all;

p.beta = 0.212; p.gamma = 0.10; p.alpha = 0.111; p.lambda = 0.0083;
p.pSI = 1.0; p.pII = 0.0;
p.pIH = 0.1060; p.pIR = 0.8921; p.pID = 0.0019;
...
Nvals = [550, 1300, 2900, 7000];
tt = linspace(0, p.tmax, 5001);

avg = cell(length(Nvals), 1);
for k = 1:length(Nvals)
    fprintf('N=%d ... ', Nvals(k));
    avg{k} = path_average(Nvals(k), p, tt);
    fprintf('done\n');
end
...
legend('Location', 'northeast', 'Interpreter', 'latex', 'FontSize', 40);
set(gca, 'LooseInset', get(gca,'TightInset'), 'FontSize', 30);
saveas(gcf, 'PSIHRS_det_id.png');

%% Path-averaged stochastic plots

for k = 1:length(avg)
    figure('Position', [100 100 1000 700]); hold on;
    plot(avg{k}.T, avg{k}.S, 'Color', '#EDB120', 'LineWidth', 4, 'DisplayName', '$\textbf{s}_N(t)$');
    plot(avg{k}.T, avg{k}.R, 'g', 'LineWidth', 4, 'DisplayName', '$\textbf{r}_N(t)$');
    xlabel('Time (days)', 'FontSize', 30);
    ylabel('$\textbf{s}_N(t)$, $\textbf{r}_N(t)$', 'Interpreter', 'latex', 'FontSize', 30);
    grid on; xlim([0 p.tmax]);
    legend('Location', 'northeast', 'Interpreter', 'latex', 'FontSize', 40);
    set(gca, 'LooseInset', get(gca,'TightInset'), 'FontSize', 30);
    saveas(gcf, sprintf('PSIHRS_stoch_N%d_sr.png', Nvals(k)));
end

for k = 1:length(avg)
    figure('Position', [100 100 1000 700]); hold on;
    plot(avg{k}.T, avg{k}.I, 'Color', '#7E2F8E', 'LineWidth', 4, 'DisplayName', '$\textbf{i}_N(t)$');
    plot(avg{k}.T, avg{k}.D, 'c', 'LineWidth', 4, 'DisplayName', '$\textbf{d}_N(t)$');
    xlabel('Time (days)', 'FontSize', 30);
    ylabel('$\textbf{i}_N(t)$, $\textbf{d}_N(t)$', 'Interpreter', 'latex', 'FontSize', 30);
    grid on; xlim([0 p.tmax]);
    legend('Location', 'northeast', 'Interpreter', 'latex', 'FontSize', 40);
    set(gca, 'LooseInset', get(gca,'TightInset'), 'FontSize', 30);
    saveas(gcf, sprintf('PSIHRS_stoch_N%d_id.png', Nvals(k)));
end

for k = 1:length(avg)
    figure('Position', [100 100 1000 700]); hold on;
    plot(avg{k}.T, avg{k}.H, 'Color', '#77AC30', 'LineWidth', 4, 'DisplayName', '$\mathbf{h}_N(t)$');
    xlabel('Time (days)', 'FontSize', 30);
    ylabel('$\mathbf{h}_N(t)$', 'Interpreter', 'latex', 'FontSize', 30);
    grid on; xlim([0 p.tmax]);
    legend('Location', 'northeast', 'Interpreter', 'latex', 'FontSize', 40);
    set(gca, 'LooseInset', get(gca,'TightInset'), 'FontSize', 30);
    saveas(gcf, sprintf('PH_only_N%d.png', Nvals(k)));
end

fprintf('\nDone.\n');

%% Path average
function a = path_average(N, p, tt)
    Ssum = zeros(size(tt)); Isum = zeros(size(tt)); Hsum = zeros(size(tt));
    Rsum = zeros(size(tt)); Dsum = zeros(size(tt));
    
    for r = 1:p.nruns
        path = run_stoch(N, p);
        Ssum = Ssum + interp1(path.T, path.S, tt, 'previous', 'extrap');
        Isum = Isum + interp1(path.T, path.I, tt, 'previous', 'extrap');
        Hsum = Hsum + interp1(path.T, path.H, tt, 'previous', 'extrap');
        Rsum = Rsum + interp1(path.T, path.R, tt, 'previous', 'extrap');
        Dsum = Dsum + interp1(path.T, path.D, tt, 'previous', 'extrap');
    end
    
    a.N = N; a.T = tt;
    a.S = Ssum/p.nruns; a.I = Isum/p.nruns; a.H = Hsum/p.nruns;
    a.R = Rsum/p.nruns; a.D = Dsum/p.nruns;
end
