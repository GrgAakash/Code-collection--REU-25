% Intersection Plot for Infected Compartment (R2)
% This script creates individual plots for each N showing I(t) vs Theoretical Threshold
% Clear visualization of when blow-up conditions are met for the infected compartment
...
for idx = 1:length(N_values)
    ...
    % Calculate the theoretical blow-up condition for INFECTED compartment:
    % G2(t) = 0 when: S(t) = [γ × (p_{IH} + p_{IR} + p_{ID})] / (β × p_{SI})
    % This is a CONSTANT threshold (doesn't depend on time-varying populations)
    theoretical_threshold = (gamma * (pIH + pIR + pID)) / (beta * pSI);
    
    % Find intersection points (where S(t) ≈ theoretical threshold)
    tolerance = 0.00001;  % 0.001% tolerance for intersection detection
...
for idx = 1:length(N_values)
    ...
    % Plot theoretical threshold as horizontal dashed line (constant)
    plot(t, repmat(results{idx}.theoretical_threshold, size(t)), '--', 'Color', [0.8, 0.2, 0.5], 'LineWidth', 2, 'DisplayName', 'Theoretical Threshold');
    ...
    title(sprintf('N = %d: S(t) vs Infected Blow-up Threshold', N));
    subtitle(sprintf('S(t) = [γ × (p_{IH} + p_{IR} + p_{ID})] / (β × p_{SI}), R0 = %.2f', R0));
    xlabel('Time'); ylabel('Population Fraction');
    grid on;
    legend('Location', 'best');
    
    % Save individual figure
    saveas(gcf, sprintf('intersection_plot_for_infected_N%d_R0_%.2f.png', N, R0));
    
    fprintf('Created infected intersection plot for N = %d\n', N);
end



% Print summary
fprintf('\n=== INFECTED COMPARTMENT INTERSECTION ANALYSIS ===\n');
fprintf('Theoretical blow-up condition for INFECTED:\n');
fprintf('  S(t) = [γ × (p_{IH} + p_{IR} + p_{ID})] / (β × p_{SI})\n');
fprintf('  S(t) = [%.6f × (%.3f + %.3f + %.3f)] / (%.3f × %.1f)\n', ...
    gamma, pIH, pIR, pID, beta, pSI);
fprintf('  S(t) = %.6f (CONSTANT THRESHOLD)\n', (gamma * (pIH + pIR + pID)) / (beta * pSI));
fprintf('\n');

fprintf('Created individual plots for each population size N\n');
fprintf('Each plot shows:\n');
fprintf('  S(t) vs Constant Infected Threshold\n\n');
...
for idx = 1:length(N_values)
    ...
    fprintf('  Plot saved as: intersection_plot_for_infected_N%d_R0_%.2f.png\n', N, R0);
    fprintf('\n');
end


fprintf('\n=== SCRIPT SAVED PERMANENTLY ===\n');
fprintf('This script is now saved as intersection_plot_for_infected.m\n');
fprintf('You can run it anytime to regenerate the infected intersection plots!\n');
