% Intersection Plot for Recovered Compartment (R4)
% This script creates individual plots for each N showing R(t) vs Theoretical Threshold
% Clear visualization of when blow-up conditions are met for the recovered compartment
...
for idx = 1:length(N_values)
    ...
    % Calculate the theoretical blow-up condition for RECOVERED compartment:
    % G4(t) = 0 when: R(t) = (γ × p_{IR} × I(t) + α × p_{HR} × H(t)) / λ
    % This is a TIME-VARYING threshold that depends on both I(t) and H(t)
    theoretical_threshold = (gamma * pIR * I_interp + alpha * pHR * H_interp) / lambda;
    
    % Find intersection points (where R(t) ≈ theoretical threshold)
    tolerance = 0.001;  % 0.001% tolerance for intersection detection
    intersection_points = abs(R_interp - theoretical_threshold) <= tolerance;
    ...
    results{idx}.H_interp = H_interp;
    results{idx}.R_interp = R_interp;
    results{idx}.D_interp = D_interp;
...
for idx = 1:length(N_values)
    ...
    hold on;
    % Plot R(t) as solid line
    plot(t, results{idx}.R_interp, 'Color', [0.2, 0.5, 0.8], 'LineWidth', 2, 'DisplayName', 'R(t)');
    % Plot theoretical threshold as dashed line (time-varying)
    ...
    if ~isempty(results{idx}.intersection_indices)
        plot(t(results{idx}.intersection_indices), results{idx}.R_interp(results{idx}.intersection_indices), 'o', ...
            'Color', [0.9, 0.6, 0.1], 'MarkerSize', 8, 'MarkerFaceColor', [0.9, 0.6, 0.1], ...
            'DisplayName', 'Intersections');
    end
    
    title(sprintf('N = %d: R(t) vs Recovered Blow-up Threshold', N));
    subtitle(sprintf('R(t) = (γ × p_{IR} × I(t) + α × p_{HR} × H(t)) / λ, R0 = %.2f', R0));
    xlabel('Time'); ylabel('Population Fraction');
    grid on;
    legend('Location', 'best');
    
    % Save individual figure
    saveas(gcf, sprintf('intersection_plot_for_recovered_N%d_R0_%.2f.png', N, R0));
    
    fprintf('Created recovered intersection plot for N = %d\n', N);
end



% Print summary
fprintf('\n=== RECOVERED COMPARTMENT INTERSECTION ANALYSIS ===\n');
fprintf('Theoretical blow-up condition for RECOVERED:\n');
fprintf('  R(t) = (γ × p_{IR} × I(t) + α × p_{HR} × H(t)) / λ\n');
fprintf('  R(t) = (%.6f × %.3f × I(t) + %.1f × %.4f × H(t)) / %.4f\n', ...
    gamma, pIR, alpha, pHR, lambda);
fprintf('  R(t) = (%.6f × I(t) + %.6f × H(t)) / %.4f\n', gamma * pIR, alpha * pHR, lambda);
fprintf('  R(t) = %.6f × I(t) + %.6f × H(t) (TIME-VARYING THRESHOLD)\n', ...
    (gamma * pIR) / lambda, (alpha * pHR) / lambda);
fprintf('\n');

fprintf('Created individual plots for each population size N\n');
fprintf('Each plot shows:\n');
fprintf('  R(t) vs Time-Varying Recovered Threshold\n\n');
...
for idx = 1:length(N_values)
    ...
    fprintf('  Plot saved as: intersection_plot_for_recovered_N%d_R0_%.2f.png\n', N, R0);
    fprintf('\n');
end


fprintf('\n=== SCRIPT SAVED PERMANENTLY ===\n');
fprintf('This script is now saved as intersection_plot_for_recovered.m\n');
fprintf('You can run it anytime to regenerate the recovered intersection plots!\n');
