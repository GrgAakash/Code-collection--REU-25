% Intersection Plot for Recovered Compartment (R3)
% This script creates individual plots for each N showing H(t) vs Theoretical Threshold
...
for idx = 1:length(N_values)
    ...
    % G3(t) = 0 when: H(t) = (γ × p_{IH} × I(t)) / (α × (p_{HR} + p_{HD}))
    % This is a TIME-VARYING threshold that depends on I(t)
    theoretical_threshold = (gamma * pIH * I_interp) ./ (alpha * (pHR + pHD));
    
    % Find intersection points (where H(t) ≈ theoretical threshold)
    tolerance = 0.00001;  % tolerance for intersection detection
    intersection_points = abs(H_interp - theoretical_threshold) <= tolerance;
...
for idx = 1:length(N_values)
    ...
    hold on;
    % Plot H(t) as solid line
    plot(t, results{idx}.H_interp, 'Color', [0.2, 0.5, 0.8], 'LineWidth', 2, 'DisplayName', 'H(t)');
    ...
    if ~isempty(results{idx}.intersection_indices)
        plot(t(results{idx}.intersection_indices), results{idx}.H_interp(results{idx}.intersection_indices), 'o', ...
            'Color', [0.9, 0.6, 0.1], 'MarkerSize', 8, 'MarkerFaceColor', [0.9, 0.6, 0.1], ...
            'DisplayName', 'Intersections');
    end
    
    title(sprintf('N = %d: H(t) vs Hospitalized Blow-up Threshold', N));
    subtitle(sprintf('H(t) = (γ × p_{IH} × I(t)) / (α × (p_{HR} + p_{HD})), R0 = %.2f', R0));
    xlabel('Time'); ylabel('Population Fraction');
    grid on;
    legend('Location', 'best');
    
    % Save individual figure
    saveas(gcf, sprintf('intersection_plot_for_hospitalized_N%d_R0_%.2f.png', N, R0));
...
fprintf('  H(t) = (γ × p_{IH} × I(t)) / (α × (p_{HR} + p_{HD}))\n');
fprintf('  H(t) = (%.6f × %.3f × I(t)) / (%.1f × (%.4f + %.4f))\n', ...
    gamma, pIH, alpha, pHR, pHD);
fprintf('  H(t) = (%.6f × I(t)) / %.6f\n', gamma * pIH, alpha * (pHR + pHD));
fprintf('  H(t) = %.6f × I(t) (TIME-VARYING THRESHOLD)\n', (gamma * pIH) / (alpha * (pHR + pHD)));
fprintf('\n');

fprintf('Created individual plots for each population size N\n');
fprintf('Each plot shows:\n');
fprintf('  H(t) vs Time-Varying Hospitalized Threshold\n\n');
...
for idx = 1:length(N_values)
    ...
    fprintf('  Plot saved as: intersection_plot_for_hospitalized_N%d_R0_%.2f.png\n', N, R0);
    fprintf('\n');
end
