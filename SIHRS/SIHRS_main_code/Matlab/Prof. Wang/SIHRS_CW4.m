...
params = struct(...
    ...
    % Cwang: so I found that we can not add more numbers; it only supports
    % three numbers. but we can change these three numbers however we want.
    % I am just thinking we do not need to mimic Li Xia anymore. 


    N_values = [300,   3000, 10000];
...
function plot_comparison(results, N_values, det_result, params)
    ...
    lgd = legend(plot_handles, [arrayfun(@(x) sprintf('N=%d', x), N_values, 'UniformOutput', false), {'Deterministic'}], ...
        'Orientation', 'horizontal', 'Location', 'southoutside', 'FontSize', 30);
    ...
    %CWang: I will only need SIR so I am going to do some deletion below

    plot(det_result.T, det_result.S_prop, 'k-', 'LineWidth',  4, 'DisplayName', '$s(t)$');
    plot(det_result.T, det_result.I_prop, 'm-', 'LineWidth',  4, 'DisplayName', '$i(t)$');
   % plot(det_result.T, det_result.H_prop, 'LineWidth', 2, 'DisplayName', 'Hospitalized');
    plot(det_result.T, det_result.R_prop, 'c-', 'LineWidth',   4, 'DisplayName', '$r(t)$');
   % plot(det_result.T, det_result.D_prop, 'LineWidth', 2, 'DisplayName', 'Dead');
    
    % Customize the plot
    xlabel('Time (days)', 'FontSize', 30);
%CWang: I need to change it to latex
    %ylabel('$s(t)$', 'FontSize', 14);

%ylabel('$\hat{\psi}$','Interpreter','latex', 'FontSize',14)
%ylabel('$\hat{\psi}$','Interpreter','latex', 'FontSize',14)
ylabel('$s(t)$, $i(t)$, $r(t)$','Interpreter','latex', 'FontSize',30)
...
set(gca,'LooseInset',get(gca,'TightInset'))
   ...
   % title('Deterministic SIHRS with Death Model Dynamics', 'FontSize', 16);
    ...
    for i = 1:length(results)
        ...
        plot(results{i}.T, results{i}.S_prop, 'b-', 'LineWidth', 4, 'DisplayName', '$\textbf{s}_N(t)$');
        hold on;
        plot(results{i}.T, results{i}.I_prop, 'r-', 'LineWidth', 4, 'DisplayName', '$\textbf{i}_N(t)$');
   %     plot(results{i}.T, results{i}.H_prop, 'm-', 'LineWidth', 2, 'DisplayName', 'Hospitalized (H)');
        plot(results{i}.T, results{i}.R_prop, 'g-', 'LineWidth', 4, 'DisplayName', '$\textbf{r}_N(t)$');
   %     plot(results{i}.T, results{i}.D_prop, 'k-', 'LineWidth', 2, 'DisplayName', 'Dead (D)');
     
        % Customize the plot
        xlabel('Time (days)', 'FontSize', 20);
%        ylabel('Proportion', 'FontSize', 12);
%% Below is the an example using Latex for y-label
ylabel('$\textbf{s}_N(t)$, $\textbf{i}_N(t)$, $\textbf{r}_N(t)$','Interpreter','latex', 'FontSize', 20);
      ...
      %  title(sprintf('SIHRS Stochastic Model - N = %d', N_values(i)), 'FontSize', 14);
        ...
        %CWang: I have commented them out
...
%%% The following loop is the attempt to generate the short term figures
%%% for H(t) only, which will serve as an intro for the long term case
    for i = 1:length(results)
        % Create figure for this N value
        figure('Position', [100, 100, 1000, 700]);
% Try out to plot H, I for IPC and deterministic model
%  plot(results{i}.T,results{i}.H_prop,det_result.T, det_result.H_prop,results{i}.T,results{i}.I_prop,det_result.T, det_result.I_prop)   
    
    plot(results{i}.T,results{i}.H_prop,'Color', '[1 0.5 0]', 'LineWidth',  3, 'DisplayName', '$\textbf{h}_N(t)$')
    hold on
    plot(det_result.T, det_result.H_prop,'Color', det_color,  'LineWidth',  3, 'DisplayName', '$h(t)$')
    
    
  xlabel('Time (days)', 'FontSize', 30);
% 
ylabel('$h(t)$, $\textbf{h})N(t)$','Interpreter','latex', 'FontSize',30)
...
%CWagn: this makes the whtie padding outside of the plots vanish
set(gca,'LooseInset',get(gca,'TightInset'))


 saveas(gcf, sprintf('H_N%d.png', N_values(i)));


    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% i am adding more plots here
...
set(gca,'LooseInset',get(gca,'TightInset'))
 ...
 % Save the deterministic plot
    saveas(gcf, 'SIHRS_deterministic_only_sr.png');
    


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%adding even more plots
...
%        ylabel('Proportion', 'FontSize', 12);
%% Below is the an example using Latex for y-label
ylabel('$\textbf{s}_N(t)$,  $\textbf{r}_N(t)$','Interpreter','latex', 'FontSize', 20);
      ...
      %  ylim([0, 1]);
        ...
        % Save the figure
        saveas(gcf, sprintf('SIHRS_stochastic_N%d_sr.png', N_values(i)));
    end
...
%% Cwang: adding more plots
 ...
 % Create separate figures for each population size N
   ...
   %     plot(results{i}.T, results{i}.H_prop, 'm-', 'LineWidth', 2, 'DisplayName', 'Hospitalized (H)');
     %   plot(results{i}.T, results{i}.R_prop, 'g-', 'LineWidth', 4, 'DisplayName', '$\textbf{r}_N(t)$');
        plot(results{i}.T, results{i}.D_prop, 'y-', 'LineWidth', 4, 'DisplayName', '$\textbf{d}_N(t)$');
     
        % Customize the plot
        xlabel('Time (days)', 'FontSize', 20);
%        ylabel('Proportion', 'FontSize', 12);
%% Below is the an example using Latex for y-label
ylabel('$\textbf{i}_N(t)$, $\textbf{d}_N(t)$','Interpreter','latex', 'FontSize', 20);
     ...
     %   text(0.02, 0.98, param_text, 'Units', 'normalized', 'FontSize', 10, ...
       %     'VerticalAlignment', 'top', 'BackgroundColor', 'white');
        ...
        % Save the figure
        saveas(gcf, sprintf('SIHRS_stochastic_N%d_id.png', N_values(i)));
    end
...
%%CWang: adding more plots
 ...
 %   plot(det_result.T, det_result.S_prop, 'k-', 'LineWidth',  4, 'DisplayName', '$s(t)$');
   ...
   % plot(det_result.T, det_result.H_prop, 'LineWidth', 2, 'DisplayName', 'Hospitalized');
   % plot(det_result.T, det_result.R_prop, 'c-', 'LineWidth',   4, 'DisplayName', '$r(t)$');
    plot(det_result.T, det_result.D_prop, 'LineWidth', 4, 'DisplayName', '$d(t)$');
...
set(gca,'LooseInset',get(gca,'TightInset'))
   ...
   % title('Deterministic SIHRS with Death Model Dynamics', 'FontSize', 16);
    ...
    % Save the deterministic plot
    saveas(gcf, 'SIHRS_deterministic_only_id.png');
