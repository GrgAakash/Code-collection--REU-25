...
# Read the data files for Washington, MS
active_cases_df = CSV.read("washington_mississippi_active_cases.csv", DataFrame)
daily_deaths_df = CSV.read("washington_mississippi_daily_deaths.csv", DataFrame)
...
CSV.write("washington_ms_pid_simple.csv", result_df)

println("Simplified data saved to washington_ms_pid_simple.csv")
println("Columns:")
println("- date: Date")
...
for i in 1:nrow(result_df)
    if result_df.active_cases[i] > 10 && result_df.daily_deaths[i] > 0 && count_examples < 3
