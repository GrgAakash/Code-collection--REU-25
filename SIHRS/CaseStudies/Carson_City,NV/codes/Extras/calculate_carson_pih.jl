...
combined_df = CSV.read("carson_city_hospitalization_active_cases_combined.csv", DataFrame)
...
CSV.write("carson_city_pih_lagged_14days.csv", pih_data)

println("P(IH) calculation data saved to carson_city_pih_lagged_14days.csv")
println("Columns:")
...
if nrow(valid_pih) > 0
    println("\n" * "="^60)
    println("P(IH) SUMMARY STATISTICS - CARSON CITY, NV")
    println("="^60)
