...
% - p2: Recovery or Hospitalization probability
% - ph: hospitalization probability
% - p3: recovery probability from hospitalized
% - gamma: infection to Recovery or hospitalization rate
% - alpha: hospitalized to recovery rate
...
% - V3 = (1/N) * (p2 * ph * gamma * I_prop + alpha * ph * H_prop)
% - V4 = (1/N) * (p2 * (1-ph) * gamma * I_prop + alpha * ph * H_prop)   (recovered)
clear all;
close all;
function sihr_simulation_improved()
    % Set random seed for reproducibility
    rng(1);
    
    % Centralized parameters
    params.p1 = 0.5;              % Infection probability
    params.p2 = 0.5;              % probability of infection to (Hospitalizarion or Recovery)
    params.ph = 0.5;              % probability of hospitalization
    params.p3 = 0.5;              % Recovery from hospitalization probability
    params.gamma = 1;             % Infection to (Hospitalizarion or Recovery) rate
    params.alpha = 1;             % Hospitalized to Recovery rate
...
function simulate_and_analyze(params, R0)
    ...
    beta = R0 * params.gamma ; % should be * p2/p1 but avoided since they are same and ode function is written that way
...
function result = run_multiple_gillespie(N, beta, params, t)
    ...
    for run = 1:params.n_runs
        [S_hist, I_hist, H_hist, R_hist, time_pts] = gillespie_sim(N, beta, params);
        [S_interp, I_interp, H_interp, R_interp] = interpolate_results(S_hist, I_hist, H_hist, R_hist, time_pts, t, N);
        S_all(run, :) = S_interp;
        I_all(run, :) = I_interp;
        H_all(run, :) = H_interp;
        R_all(run, :) = R_interp;
    ...
    V3 = (1/N) * (params.p2 * params.ph * params.gamma * result.I_mean + params.p3 * params.alpha * result.H_mean);
    V4 = (1/N) * (params.p2 * (1-params.ph) * params.gamma * result.I_mean + params.p3 * params.alpha * result.H_mean);

    
    result.sigma1 = sqrt(V1);
    result.sigma2 = sqrt(V2);
    result.sigma3 = sqrt(V3);
    result.sigma4 = sqrt(V4);
    result.N = N;
...
function [S_hist, I_hist, H_hist, R_hist, time_pts] = gillespie_sim(N, beta, params)
    ...
    % Gillespie algorithm
    while current_time < params.T && (I > 0 || H > 0)
        si_rate = (beta / N) * S * I; % rate of S->I
        ihr_rate = params.gamma * I; % rate of I->(H U R)
        hr_rate = params.alpha * H ; % rate of H->R
        event_rate = si_rate+ihr_rate+hr_rate;
        
        if event_rate <= 0
            break;
        end
        
        tau = -log(rand) / event_rate;
        ...
        a = rand;
        if a < si_rate / event_rate   % i-clock advances first
            if rand<params.p1 && S>0  % S->I
                S = S - 1;
                I = I + 1;
            end
        elseif a < (si_rate+ihr_rate)/event_rate % r-clock advances first
            if rand < params.p2 && I > 0 % Hospitalization or Recovery happens
                if rand < params.ph 
                    I = I - 1;
                    H = H + 1;
                else 
        ...
        else                            % h-clock advances
            if rand<params.p3 && H>0    % H->R
                H = H-1;
                R = R+1;
            end
        end
    ...
    % Verify population conservation
    assert(all(S_hist + I_hist + H_hist + R_hist == N), 'Population not conserved');
end

function [S_interp, I_interp, H_interp, R_interp] = interpolate_results(S_hist, I_hist, H_hist, R_hist, time_pts, t, N)
...
function det_result = solve_deterministic_sir(beta, params)
    ...
    try
        tspan = [0, params.T];
        y0 = [params.initial_s; params.initial_i; params.initial_h; params.initial_r];
        ode_system = @(t, y) [
        -params.p1*beta*y(1)*y(2);           % ds/dt
        params.p1*beta*y(1)*y(2) - params.p2*params.gamma*y(2);  % di/dt
        params.ph*params.p2*params.gamma*y(2) - params.alpha*params.p3*y(3); % dh/dt
        (1-params.ph)*params.p2*params.gamma*y(2) + params.alpha*params.p3*y(3)           % dr/dt
    ];
...
function plot_results(t, results, det_result, params, R0)
    ...
    % Plot sigma3 (hospitalized)
    nexttile;
    hold on;
    for idx = 1:length(params.N_values)
        plot(t, results{idx}.sigma3, 'Color', params.colors{idx}, 'LineWidth', 1.5);
    end
    title(sprintf('Hospitalized Std Dev (R_0 = %.2f)', R0));
    xlabel('Time'); ylabel('\sigma_N^{(3)}(t)');
    grid on;

    % Plot sigma4 (recovered)
    nexttile;
    hold on;
    for idx = 1:length(params.N_values)
        plot(t, results{idx}.sigma4, 'Color', params.colors{idx}, 'LineWidth', 1.5);
    end
    title(sprintf('Recovered Std Dev (R_0 = %.2f)', R0));
    xlabel('Time'); ylabel('\sigma_N^{(4)}(t)');
    grid on;
