...
function result = sir_agent_model(N)
    ...
    gamma = 1;      % recovery constant
    p1 = 0.5;       % odds of infection
    p2 = 0.5;       % odds of recovery
    ...
    while ~isempty(I) && t < 25
        ...
        if t > 25
            t = 25; % Cap time at 25
            T(end+1) = t;
            current_total = numel(S) + numel(I) + numel(R);
...
function det_result = solve_deterministic_sir()
    ...
    % The effective rates should account for the probability of events occurring
    effective_beta = p1 * beta;
    effective_gamma = p2 * gamma;
    
    ode_system = @(t, y) [
        -effective_beta * y(2) * y(1);           % ds/dt
        effective_beta * y(2) * y(1) - effective_gamma * y(2);  % di/dt
        effective_gamma * y(2)                   % dr/dt
    ];
